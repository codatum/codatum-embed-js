...
describe("createParamMapper", () => {
  ...
  it("encode omits keys with null or undefined value", () => {
    ...
    expect(result[0]).toEqual({
      param_id: paramDefs.store_id.paramId,
      param_value: '"x"',
    });
  });
  ...
  it("encode sets is_hidden when paramDef has hidden: true", () => {
    ...
    const storeParam = result.find((p) => p.param_id === defsWithHidden.store_id.paramId);
    expect(storeParam?.is_hidden).toBe(true);
    const dateParam = result.find((p) => p.param_id === defsWithHidden.date_range.paramId);
  ...
  it("encode uses RESET_TO_DEFAULT string as param_value when value is RESET_TO_DEFAULT", () => {
    const mapper = createParamMapper({ key: { paramId: "k1" } });
  ...
  it("decode maps param_id to alias and JSON.parses value", () => {
    ...
    const params: EncodedParam[] = [
      { param_id: paramDefs.store_id.paramId, param_value: '"store_002"' },
      { param_id: paramDefs.date_range.paramId, param_value: '["2025-02-01","2025-02-28"]' },
    ];
  ...
  it("decode ignores param_id not in paramDefs", () => {
    ...
    const params: EncodedParam[] = [
      ...
      { param_id: paramDefs.store_id.paramId, param_value: '"x"' },
    ];
    const decoded = mapper.decode(params);
