...
- **cdn.html** — Standalone HTML that loads the embed script from the local build (`../../packages/embed/dist/index.global.min.js`). Same flow: `/config` → init with `/token` as token provider. Can also be opened as a file or served by another static server if the script path is valid.
